...
export default function App() {
  ...
  <button onClick={() => setSubTabAbsen('input')} className={`flex-1 py-2 text-xs font-bold rounded-lg ${subTabAbsen === 'input' ? 'bg-white shadow' : ''}`}>Absen Harian</button>
  <button onClick={() => setSubTabAbsen('rekap')} className={`flex-1 py-2 text-xs font-bold rounded-lg ${subTabAbsen === 'rekap' ? 'bg-white shadow' : ''}`}>Rekap Bulanan</button>
  </div>
  
  {subTabAbsen === 'input' ? (
  <div className="space-y-4">
  <div className="bg-white p-4 rounded-xl border shadow-sm">
  <select className="w-full border p-2 rounded-lg text-sm mb-2" onChange={(e) => setRekapKelas(e.target.value)}>
  <option value="">Pilih Kelas</option>
  {[...new Set(muridList.map(m => m.kelas))].map(k => <option key={k} value={k}>{k}</option>)}
  </select>
  <div className="text-[10px] text-slate-500 font-bold uppercase">Absensi: {new Date().toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</div>
  </div>
  
  {muridList.filter(m => m.kelas === rekapKelas).map(m => (
  <div key={m.id} className="bg-white p-3 rounded-xl border flex items-center justify-between">
  <span className="text-sm font-medium">{m.nama}</span>
  <div className="flex gap-1">
  {['H', 'I', 'A'].map(status => (
  <button key={status} onClick={() => setTempAbsen({...tempAbsen, [m.id]: status})} 
  className={`w-8 h-8 rounded-lg text-xs font-bold ${tempAbsen[m.id] === status ? 'bg-emerald-600 text-white' : 'bg-slate-100'}`}>
  {status}
  </button>
  ))}
  </div>
  ...
  <button onClick={() => { /* Masukkan fungsi supabase.from('absensi').insert(...) di sini */ alert('Data Absen Disimpan!'); }} 
  className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold shadow-lg">
  <i className="fa-solid fa-save mr-2"></i> Simpan Absensi Hari Ini
  </button>
  </div>
  ) : (
  <div className="space-y-4">
  <div className="bg-white p-4 rounded-xl border shadow-sm">
  <input type="month" className="w-full border p-2 rounded-lg text-sm mb-2" onChange={(e) => setRekapBulan(e.target.value)} />
  <select className="w-full border p-2 rounded-lg text-sm mb-3" onChange={(e) => setRekapKelas(e.target.value)}>
  <option value="">Pilih Kelas</option>
  {[...new Set(muridList.map(m => m.kelas))].map(k => <option key={k} value={k}>{k}</option>)}
  </select>
  <button onClick={() => exportAbsenPDF(rekapBulan, rekapKelas)} className="w-full bg-red-600 text-white py-2 rounded-lg text-sm font-bold mb-2">
  <i className="fa-solid fa-file-pdf mr-2"></i> Export PDF Rekap
  </button>
  <button onClick={() => shareAbsenWA(rekapBulan, rekapKelas)} className="w-full bg-green-600 text-white py-2 rounded-lg text-sm font-bold">
  <i className="fa-brands fa-whatsapp mr-2"></i> Share WA Rekap
  </button>
  </div>
  </div>
  )}
  ...
  {/* Form Rekap Bulanan */}
  <input type="month" className="w-full border p-2 rounded-lg text-sm mb-2" onChange={(e) => setRekapBulan(e.target.value)} />
  <button onClick={() => downloadRekapPDF(rekapBulan)} className="w-full bg-red-600 text-white py-2 rounded-lg text-sm font-bold mb-2">Export PDF Rekap</button>
  <button onClick={() => shareRekapWA(rekapBulan)} className="w-full bg-green-600 text-white py-2 rounded-lg text-sm font-bold">Share WA Rekap</button>
  </div>
  </div>
  )}
  </div>
