...
export default function App() {
  ...
  <main className="flex-1 p-4 md:p-6 overflow-x-hidden">
        ...
        a: absenMuridTerkait.filter(x => x.status === 'Alpa').length,
        };
        });
        ...
        <button 
        onClick={() => setAbsenTab('rekap')} 
        className={`flex-1 py-2.5 text-xs font-black uppercase tracking-wide rounded-xl transition-all duration-300 ${absenTab === 'rekap' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
        >
        Rekapitulasi
        </button>
        </div>
        ...
        >I</button>
        <button 
        onClick={() => setStatusAbsen(m.id, 'Alpa')} 
        className={`w-9 h-9 rounded-lg text-xs font-black transition-all ${statusAktif === 'Alpa' || statusAktif === 'Alfa' ? 'bg-rose-500 text-white shadow-md shadow-rose-500/30' : 'text-slate-400 hover:bg-slate-100'}`}
        >A</button>
        </div>
        </div>
        );
