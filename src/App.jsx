...
export default function App() {
  ...
  // === STATE ABSENSI BARU (ROMBAK TOTAL) ===
  ...
  const exportRekapAbsenPDF = (bulanTahun, kelas) => {
    ...
    const yTable = buatHeaderPDF(doc, "ARSIP PRESENSI BULANAN SANTRI", [
      `Kelas         : ${kelas}`,
      `Periode       : ${namaBulan}`,
    ...
    muridDiKelas.forEach(m => {
      ...
      tableData.push([m.nama, hadir, izin, alfa]);
    });

    doc.autoTable({
      startY: yTable,
      head: [['Nama Santri', 'Hadir (H)', 'Izin (I)', 'Alfa (A)']],
      body: tableData,
      theme: 'grid',
  ...
  const shareRekapAbsenWA = (bulanTahun, kelas) => {
    const namaBulan = new Date(bulanTahun + "-01").toLocaleDateString('id-ID', { month: 'long', year: 'numeric' });
    let text = `*📄 ARSIP PRESENSI BULANAN*\n\n` +
               `🏫 *Kelas* : ${kelas}\n` +
               `🗓️ *Periode* : ${namaBulan}\n` +
               `👨‍🏫 *Pengajar* : ${user?.email || 'Ustaz'}\n` +
               `➖➖➖➖➖➖➖➖➖➖\n\n`;
  ...
  const exportNilaiKelasPDF = (kelas) => {
    ...
    muridList.filter(m => m.kelas === kelas).forEach(m => {
      ...
      if (nilaiMurid.length === 0) {
         ...
         nilaiMurid.forEach((n, idx) => {
            tableData.push([idx === 0 ? m.nama : '', n.pelajaran, n.jenis_ujian, `${n.skor} Pts`]);
         });
      }
    });
    ...
    doc.autoTable({
      startY: yTable,
      head: [['Nama Santri', 'Mata Pelajaran', 'Jenis Ujian', 'Skor']],
      body: tableData,
      theme: 'grid',
  ...
  const shareNilaiKelasWA = (kelas) => {
    let text = `*📊 REKAPITULASI NILAI KELAS ${kelas}*\n` +
               `👨‍🏫 *Pengajar* : ${user?.email || 'Ustaz'}\n` +
  ...
  const exportRaporPDF = (murid) => {
    ...
    const tableData = nilais.map(n => [n.pelajaran, n.jenis_ujian, `${n.skor} Pts`]);

    doc.autoTable({
      startY: yTable,
      head: [['Mata Pelajaran', 'Jenis Evaluasi', 'Skor Nilai']],
      body: tableData,
      theme: 'grid',
      headStyles: { fillColor: [5, 150, 105] },
  ...
  const shareRaporWA = (murid) => {
    const nilais = listNilai.filter(n => n.murid_id === murid.id);
    let text = `*🎓 RAPOR HASIL BELAJAR SANTRI*\n\n` +
               `👤 *Nama* : ${murid.nama}\n` +
               `🏫 *Kelas* : ${murid.kelas}\n` +
               `👨‍🏫 *Pengajar* : ${user?.email || 'Ustaz'}\n` +
               `➖➖➖➖➖➖➖➖➖➖\n\n*📝 Rincian Nilai:*\n`;
  ...
  // PEMBARUAN: Simpan Absen dengan fitur Edit/Replace jika sudah ada
  const handleSimpanAbsen = async (muridId, status) => {
  const tanggalAbsen = absenTanggal || new Date().toISOString().split('T')[0];
  const mapel = absenPelajaran || '-';
  
  // Cari apakah absen sudah ada untuk murid ini di tanggal ini (Fitur Edit Absen)
  const existingAbsenIndex = listAbsensi.findIndex(a => a.murid_id === muridId && a.tanggal === tanggalAbsen);
  
  if (existingAbsenIndex >= 0) {
  // UPDATE absen yang sudah ada (Salah pencet bisa diubah langsung)
  ...
  <h2 className="text-xl font-bold text-emerald-800 mb-4 border-b pb-2">Presensi Santri</h2>
  ...
  <button onClick={()=>setAbsenTabMode('rekap')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition ${absenTabMode === 'rekap' ? 'bg-white shadow text-emerald-700' : 'text-slate-500'}`}>Arsip & Export</button>
  </div>
  ...
  <div className="bg-emerald-100 p-3 flex justify-between items-center border-b border-emerald-200">
  <span className="font-bold text-emerald-800 text-sm">Daftar Hadir Kelas {absenKelas}</span>
  <span className="text-xs bg-white px-2 py-1 rounded-md text-slate-600">{absenTanggal}</span>
  </div>
  ...
  <button onClick={() => handleSimpanAbsen(m.id, 'Hadir')} className={`flex-1 md:w-16 py-1.5 rounded-lg text-sm font-bold border transition ${statusSkrg === 'Hadir' ? 'bg-emerald-500 text-white border-emerald-600 shadow-inner' : 'bg-slate-50 text-slate-500 hover:bg-emerald-100'}`}>Hadir</button>
  <button onClick={() => handleSimpanAbsen(m.id, 'Izin')} className={`flex-1 md:w-16 py-1.5 rounded-lg text-sm font-bold border transition ${statusSkrg === 'Izin' ? 'bg-blue-500 text-white border-blue-600 shadow-inner' : 'bg-slate-50 text-slate-500 hover:bg-blue-100'}`}>Izin</button>
  <button onClick={() => handleSimpanAbsen(m.id, 'Alfa')} className={`flex-1 md:w-16 py-1.5 rounded-lg text-sm font-bold border transition ${statusSkrg === 'Alfa' ? 'bg-red-500 text-white border-red-600 shadow-inner' : 'bg-slate-50 text-slate-500 hover:bg-red-100'}`}>Alfa</button>
  </div>
  </div>
  );
  })}
  ...
  Silakan isi Tanggal, Pelajaran, dan Kelas terlebih dahulu.
  </div>
  )}
  </div>
  )}
  ...
  <input type="month" className="w-full border rounded-lg py-2 px-3 text-sm font-bold" value={rekapBulanTahun} onChange={e=>setRekapBulanTahun(e.target.value)} />
  </div>
  <div className="flex-1">
  <label className="text-xs font-bold text-slate-500 mb-1 block">Pilih Kelas</label>
  <select className="w-full border rounded-lg py-2 px-3 text-sm font-bold" value={rekapKelas} onChange={e=>setRekapKelas(e.target.value)}>
  ...
  <button onClick={()=>exportRekapAbsenPDF(rekapBulanTahun, rekapKelas)} className="flex-1 bg-red-50 text-red-700 py-2 rounded-lg text-sm font-bold border border-red-200 flex items-center justify-center gap-2 hover:bg-red-100">
  <i className="fa-solid fa-file-pdf"></i> Export PDF
  </button>
  <button onClick={()=>shareRekapAbsenWA(rekapBulanTahun, rekapKelas)} className="flex-1 bg-green-50 text-green-700 py-2 rounded-lg text-sm font-bold border border-green-200 flex items-center justify-center gap-2 hover:bg-green-100">
  ...
  <thead className="bg-emerald-50 text-emerald-800 font-bold border-b">
  <tr>
  ...
  <td className="py-3 px-2 text-center font-bold text-emerald-600 bg-emerald-50/30">{hadir}</td>
  <td className="py-3 px-2 text-center font-bold text-blue-600 bg-blue-50/30">{izin}</td>
  <td className="py-3 px-2 text-center font-bold text-red-600 bg-red-50/30">{alfa}</td>
  </tr>
  )
  })}
  </tbody>
  ...
  <span className="font-bold text-slate-700">Pilih Santri</span>
  <div className="flex gap-2">
  <button onClick={()=>exportNilaiKelasPDF(nilaiKelas)} className="text-red-600 bg-red-100 px-2 py-1 rounded text-xs font-bold"><i className="fa-solid fa-file-pdf"></i></button>
  <button onClick={()=>shareNilaiKelasWA(nilaiKelas)} className="text-green-600 bg-green-100 px-2 py-1 rounded text-xs font-bold"><i className="fa-brands fa-whatsapp"></i></button>
  </div>
  </div>
  <div className="divide-y">
  {muridList.filter(m=>m.kelas===nilaiKelas).map(m=>(
  <button key={m.id} onClick={()=>setSelectedMuridNilai(m)} className="w-full text-left p-4 hover:bg-emerald-50 flex justify-between items-center group">
  ...
  <div className="space-y-4">
  <div className="flex gap-2">
  <button onClick={()=>setSoalTab('arsip')} className={`flex-1 py-2 rounded-lg text-sm font-bold ${soalTab === 'arsip' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-600 border'}`}>Arsip Soal</button>
  <button onClick={()=>{setFormSoal({id:null, pelajaran:'', kelas:'', batasan:'', isi:''}); setSoalTab('buat');}} className={`flex-1 py-2 rounded-lg text-sm font-bold ${soalTab === 'buat' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-600 border'}`}>+ Buat Soal</button>
  </div>
  ...
  <button type="submit" className="w-full bg-emerald-600 text-white font-bold py-2.5 rounded-xl">Simpan ke Bank Soal</button>
  </form>
  )}
  ...
  <div className="flex gap-2">
  <button onClick={()=>exportSoalPDF(s)} className="flex-1 bg-red-50 text-red-700 py-1.5 rounded text-xs font-bold border border-red-100 flex justify-center items-center gap-1"><i className="fa-solid fa-file-pdf"></i> Cetak PDF</button>
  <button onClick={()=>shareSoalWA(s)} className="flex-1 bg-green-50 text-green-700 py-1.5 rounded text-xs font-bold border border-green-100 flex justify-center items-center gap-1"><i className="fa-brands fa-whatsapp text-sm"></i> Share WA</button>
  </div>
  </div>
  ))}
  ...
  <button key={m.id} onClick={()=>setSelectedMuridRapor(m)} className="w-full text-left p-4 hover:bg-slate-50 font-semibold text-slate-700 flex justify-between">
  ...
  <div key={n.id} className="flex justify-between text-sm border-b pb-1">
  <span className="text-slate-600">{n.pelajaran} ({n.jenis_ujian})</span>
  <span className="font-bold">{n.skor}</span>
  </div>
  ))}
  </div>
  
  <div className="flex gap-2">
  <button onClick={()=>exportRaporPDF(selectedMuridRapor)} className="flex-1 bg-red-600 hover:bg-red-700 text-white py-2 rounded-xl text-sm font-bold flex justify-center items-center gap-2"><i className="fa-solid fa-file-pdf"></i> Download Rapor</button>
  <button onClick={()=>shareRaporWA(selectedMuridRapor)} className="flex-1 bg-green-500 hover:bg-green-600 text-white py-2 rounded-xl text-sm font-bold flex justify-center items-center gap-2"><i className="fa-brands fa-whatsapp text-lg"></i> Kirim ke Wali</button>
  </div>
  </div>
  )}
  </div>
  ...
  <p className="text-slate-500 text-sm">Fitur {nilaiModeTab} sedang dalam mode ringkas. Navigasi sama seperti versi sebelumnya.</p>
  </div>
  )}
  ...
  <button type="submit" className="w-full bg-emerald-600 text-white font-bold py-2 rounded-lg text-sm">Simpan Catatan</button>
  </form>
  ...
  <button type="submit" className="w-full bg-emerald-600 text-white font-bold py-2 rounded-lg text-sm">Simpan Tagihan</button>
  </form>
  </div>
  )}
  </div>
  ...
  <div className={`p-3 rounded-full -mt-8 shadow-lg border-4 border-white text-white ${activeTab==='absen' ? 'bg-emerald-600' : 'bg-slate-400'}`}>
