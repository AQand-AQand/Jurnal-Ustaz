...
export default function App() {
  ...
  const [soalTab, setSoalTab] = useState('arsip'); // Baru: Tab Arsip/Buat Soal
  ...
  const [showTambahJadwal, setShowTambahJadwal] = useState(false); // Baru: Toggle Jadwal
  ...
  // Data untuk pemberitahuan jadwal
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans max-w-md mx-auto shadow-2xl border-x border-slate-200 overflow-hidden pb-32 relative">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-5 space-y-5">
        ...
        {activeTab === 'jadwal' && (
          <div className="space-y-4">
            ...
            {/* RUNNING TEXT / PEMBERITAHUAN */}
            <div className="bg-emerald-800 text-white rounded-2xl shadow-md overflow-hidden border border-emerald-700 flex items-center">
              <div className="bg-emerald-900 px-3 py-2 text-xs font-bold border-r border-emerald-700 z-10 whitespace-nowrap flex items-center gap-2">
                <i className="fa-solid fa-bell animate-bounce text-amber-400"></i> Info
              </div>
              <div className="py-2 text-xs font-medium w-full overflow-hidden flex items-center">
                <marquee className="w-full" scrollamount="4">
                  Selamat {ucapanWaktu}, Ustaz! Hari ini {hariIni}. 
                  {jdwlHariIni.length > 0 ? ` Anda memiliki ${jdwlHariIni.length} jadwal pelajaran hari ini.` : ` Tidak ada jadwal mengajar rutin hari ini.`}
                  {tagihanHariIni.length > 0 && ` ⚠️ PERHATIAN: Ada ${tagihanHariIni.length} janji ulangan/muhafadhoh hari ini!`}
                </marquee>
              </div>
            </div>
        ...
        {activeTab === 'rapor' && (
          <div className="space-y-4">
            ...
            {/* Tampilan Rapor (Tetap Sama Seperti Sebelumnya) */}
            {raporKelas && selectedMuridRapor && (() => {
        ...
        {activeTab === 'nilai' && (
          <div className="space-y-4">
             ...
             {/* Sub menu Nilai & Hafalan sama persis dengan yang sebelumnya */}
             {nilaiModeTab === 'ujian' && (
                <div className="space-y-4">
                  {!nilaiKelas && (
                    <div className="bg-white p-5 rounded-2xl border shadow-sm space-y-4">
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white border-t flex justify-between px-2 py-2 z-50 shadow-[0_-2px_8px_rgba(0,0,0,0.06)]">
        {[
          ...
          { id: 'rapor', icon: 'fa-users', label: 'Santri' }, // Icon diganti agar merepresentasikan manajemen santri
