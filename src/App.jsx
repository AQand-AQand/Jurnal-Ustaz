...
export default function App() {
  ...
  <button onClick={() => setSubTabAbsen('input')} className={`flex-1 py-2 text-xs font-bold rounded-lg ${subTabAbsen === 'input' ? 'bg-white shadow' : ''}`}>Absensi Harian</button>
  ...
  <label className="text-[10px] font-bold text-slate-500">Pilih Kelas</label>
  <select className="w-full border p-2 rounded-lg text-sm mb-2" onChange={(e) => setRekapKelas(e.target.value)}>
  <option value="">Pilih Kelas</option>
  {/* Pastikan listKelasUnik sudah terdefinisi di state */}
  ...
  {listJadwal.filter(j => j.kelas === rekapKelas).map(j => <option key={j.id}>{j.pelajaran}</option>)}
  </select>
  </div>
  
  {/* List Santri & Tombol Simpan */}
  <div className="space-y-2">
  ...
  <div className="flex gap-2">
  {['Hadir', 'Izin', 'Alfa'].map(s => (
  <button key={s} onClick={() => updateStatusAbsenLocal(m.id, s)} 
  className={`px-3 py-1 rounded text-[10px] font-bold ${tempAbsen[m.id] === s ? 'bg-emerald-600 text-white' : 'bg-slate-100'}`}>
  {s[0]}
  </button>
  ))}
  </div>
  ...
  {/* Tombol Simpan Final */}
  <button onClick={simpanAbsenKeDatabase} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold shadow-lg mt-4">
